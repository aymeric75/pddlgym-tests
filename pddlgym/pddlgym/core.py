...
class PDDLEnv(gym.Env):
    ...
    def reset(self, _problem_idx=None):
        #print("ouioui")
        """
        ...
        """
        #print("_problem_idx : {}".format(str(_problem_idx)))

        if not self._problem_index_fixed:
            self._problem_idx = self._problem_idx
            #self._problem_idx = self.rng.choice(len(self.problems))
        self._problem = self.problems[_problem_idx]
        
        #print(len(self.problems))
        #self._problem = self.problems[1]
