...
def reduce_resolution(image):
    ...
    # plt.imsave("HANOI-tmp.png", image)
    # plt.close()



    # img = cv2.imread("HANOI-tmp.png")
    res = cv2.resize(image, dsize=(120, 25), interpolation=cv2.INTER_LANCZOS4)

    # plt.imsave("HANOI-tmp.png", res)
    # plt.close()
...
def modify_one_trace(all_images_transfo_tr, all_images_orig_tr, all_actions, all_actions_unique_):
    ...
    # for ind, im in enumerate(all_images_orig_tr):

    #     plt.imsave("hanoi_"+str(ind)+".png", im)
    #     plt.close()
...
# # opens a saved dataset and augment it 
# # with images of lower resolution
# def modify_saved_dataset():

#     loaded_dataset = load_dataset("/workspace/pddlgym-tests/pddlgym/hanoi_3_9_dataset/data.p")
...
# # 2) save dataset
# save_dataset("hanoi_3_9_dataset", all_traces, obs_occurences, unique_obs_img, unique_transitions)
...
def export_dataset(action_type="semi_v1"):
    ...
    for iii, trace in enumerate(loaded_dataset["traces"]):
        ...
        for trtrtr, transitio in enumerate(trace):
            ...
            total_nb_transitions+=1
            all_images_reduced.append(reduce_resolution(transitio[0][0])) # = im1
            all_images_reduced.append(reduce_resolution(transitio[0][1])) # = im2

            # plt.imsave("hhhhanoi_pair_"+str(trtrtr)+"_pre.png", transitio[0][0])
            # plt.close()

            # exit()
            # plt.imsave("hanoi_pair_"+str(trtrtr)+"_suc.png", reduce_resolution(transitio[0][1]))
            # plt.close()
            # print("for pair {}, actio is {}".format(str(trtrtr), str(transitio[1][1])))

            # actions_for_one_trace

            if action_type == "semi_v1":
                # print(str(transitio[1][1]))
                # exit()
    ...
    print("number of transitions is: {}".format(str(total_nb_transitions)))
    ...
    reduced_uniques = []
    for uju, uniq in enumerate(unique_obs_img):
    ...
    all_images_reduced_preproc, _, _ = preprocess(all_images_reduced)

    #print(all_images_reduced_preproc.shape)
    ...
    for iiii, trace in enumerate(loaded_dataset["traces"]):
        ...
        print(len(all_images_transfo_tr))
        ...
        all_pairs_of_images_of_trace, all_pairs_of_images_orig_reduced_of_trace, actions_one_hot_of_trace = modify_one_trace(all_images_transfo_tr, all_images_orig_reduced_tr, all_actions_tr, all_actions_unique)
...
all_pairs_of_images, all_pairs_of_images_reduced_orig, all_actions_one_hot, mean_all, std_all, all_actions_unique, orig_max, orig_min = export_dataset(action_type="full")
